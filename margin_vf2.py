...
class MARGIN_VF2:
    def __init__(self, support=0.75, sortrep=False, randwalk=False) -> None:
        self.G = None
    ...
    def isomorphism(self, subgraph: nx.Graph, graph: nx.Graph):
    ...
    def support_graph(self, sub: nx.Graph) -> int:
        list_iso = [(i, self.isomorphism(sub, g)) for i, g in self.G.items()]
        list_iso = list(filter(lambda x: x[1], list_iso))
        return len(list_iso)
    ...
    def expand_cut(self, initial: Cut):
        print("Expanding cut...")
